...
NumberList::NumberList(const NumberList& original) {
    // by passing by reference, list object itself is passed (its address)
    
    // In the other hand, these things are not object itself
    // It is just a pointer which points to object
